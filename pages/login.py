...
def render_login():
    """
    Hiển thị trang đăng nhập sử dụng Google OAuth.
    """
    st.title("🌱 TerraSync IoT: Hệ Thống Nông Nghiệp Thông Minh")

    st.markdown("""
    **Tích Hợp IoT Và AI Cho Quản Lý Nước & Ruộng Bền Vững**
    TerraSync kết nối cảm biến thực địa, AI dự báo thời tiết và vệ tinh
    để giúp nông dân tối ưu tưới tiêu, phát hiện bệnh sớm và tăng năng suất.
    ---
    """)
    ...
    if not st.user.is_logged_in:
        ...
        st.success(
            f"Chào mừng trở lại, **{st.user.name or st.user.email}** 🌾")
