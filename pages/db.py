...
# Ví dụ sử dụng (không cần import streamlit ở đây)
if __name__ == "__main__":
    db = DatabaseManager()
    
    # Thêm user
    db.add("users", {"email": "kienpx@example.com", "customdatauser": "hello", "age": 25, "hobbies": ["reading", "coding"]})
    
    # Lấy tất cả users
    users = db.get("users")
    print("Users before update:", users)
    
    # Cập nhật user theo filter
    updated = db.update("users", {"email": "kienpx@example.com"}, {"customdatauser": "updated hello", "age": 30, "new_field": "added"})
    print(f"Updated {updated} user(s)")
    
    # Kiểm tra sau update
    user_after = db.get("users", {"email": "kienpx@example.com"})
    print("User after update:", user_after)
    
    # Thêm table mới, ví dụ "posts"
    db.add("posts", {"title": "Hello World", "content": "Demo post", "tags": ["demo", "json"]})
    print("Tables:", db.tables())  # ['users', 'posts']
    
    # Xóa toàn bộ table (filter=None)
    deleted_posts = db.delete("posts")
    print(f"Deleted {deleted_posts} post(s)")
    print("Tables after delete all:", db.tables())  # ['users']